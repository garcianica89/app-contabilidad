...
export default function AntiguedadCxPPage() {
  const [data, setData] = useState<any[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    api.getAntiguedadCxP().then(setData).catch(() => {}).finally(() => setLoading(false))
  }, [])

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl md:text-2xl font-bold text-white">Antiguedad de Saldos - CxP</h1>
        <p className="text-sm text-slate-400">Cuentas por pagar por rango de vencimiento</p>
      </div>

      <ChartCard title="Antiguedad CxP" subtitle="Saldos por proveedor" height={350}>
